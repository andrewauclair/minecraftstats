...
public class NBTFileHelperSpecification extends SpecTagHelper {
	...
	public void ShouldReadTagTypes() throws IOException {
		assertReadsTagType(new TAG_Short(name, (short)5));
		assertReadsTagType(new TAG_Long(name, 5L));
		assertReadsTagType(new TAG_Float(name, 5.5f));
		assertReadsTagType(new TAG_Double(name, 4.0));
		assertReadsTagType(new TAG_Byte_Array(name));
		assertReadsTagType(new TAG_String(name, "Testing"));
